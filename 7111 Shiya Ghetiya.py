...
# ---------------- GRAPH 2: PIE CHART ----------------
def company_product_comparison_chart():
    if not inventory:
        messagebox.showwarning("No Data", "No data available for graph!")
        return
...
form_frame.place(x=20, y=70, width=350, height=380)
...
btn_frame.place(x=20, y=470, width=350, height=170)

btn_style = {"font": ("Arial", 11, "bold"), "width": 14, "bd": 0, "fg": "white", "cursor": "hand2"}

# Row 1 -> Add and Delete
tk.Button(btn_frame, text="Add Item", bg="#28a745", command=add_item, **btn_style).grid(row=0, column=0, padx=8, pady=8)
tk.Button(btn_frame, text="Delete Item", bg="#dc3545", command=delete_item, **btn_style).grid(row=0, column=1, padx=8, pady=8)

# Row 2 -> Update and Qty Graph
tk.Button(btn_frame, text="Update Item", bg="#007bff", command=update_item, **btn_style).grid(row=1, column=0, padx=8, pady=8)
tk.Button(btn_frame, text="Qty Graph", bg="#6f42c1", command=product_quantity_chart, **btn_style).grid(row=1, column=1, padx=8, pady=8)

# Row 3 -> Comp Graph and Clear
tk.Button(btn_frame, text="Comp Graph", bg="#fd7e14", command=company_product_comparison_chart, **btn_style).grid(row=2, column=0, padx=8, pady=8)
tk.Button(btn_frame, text="Clear", bg="#6c757d", command=clear_fields, **btn_style).grid(row=2, column=1, padx=8, pady=8)

# Row 4 -> Exit
tk.Button(btn_frame, text="Exit", bg="#343a40", command=root.destroy, **btn_style).grid(row=3, column=0, columnspan=2, pady=10)

# ---------------- TABLE FRAME ----------------
table_frame = tk.Frame(root, bg="white", bd=2, relief=tk.RIDGE)
table_frame.place(x=390, y=70, width=730, height=570)

columns = ("ID", "Category", "Company", "Product Name", "Quantity", "Price")
...
tree.column("Product Name", width=140)
